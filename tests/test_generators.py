...
def test_filter_by_currency_rub():
    assert next(filter_by_currency(transactions, "RUB")) == {"id": 873106923,
           "state": "EXECUTED", "date": "2019-03-23T01:09:46.296404", "operationAmount": {
                "amount": "43318.34",
                "currency": {
                    "name": "руб.",
                    "code": "RUB"
...
def test_filter_by_currency():
    assert next(filter_by_currency(transactions, "USD")) == {"id": 939719570,
           "state": "EXECUTED", "date": "2018-06-30T02:08:58.425572", "operationAmount": {
                "amount": "9824.07",
                "currency": {
                    "name": "USD",
                    "code": "USD"
...
@pytest.mark.parametrize('index, expected', [(0, 'Перевод организации'), (1, 'Перевод со счета на счет'),
                                             (2, 'Перевод со счета на счет'), (3, 'Перевод с карты на карту'),
